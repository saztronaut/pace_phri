    ...
    for ($x=0; $x<=($n_targets); $x++){
    	...
    	if ($nweeks>1){
    		...
    		for ($i=1; $i<$nweeks; $i++){
    			...
    			if ($subweek==0){ $thisweek=$x*2;} else { $thisweek=($x*2)." Attempt #".($i);}}		   
    	...
    	//echo '{"targetsprint":'.json_encode($myTargetsprint). ',"targetsarray":'. json_encode($myTargetsarray).'}';
