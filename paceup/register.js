        ...
        var json = JSON.parse(result, function(key, value) {
            if (key == "success") {
              ...
              //  $('#consentModal').modal('hide');
                document.getElementById('consent_message').innerHTML = "<p> Welcome to PACE-UP Next Steps. If you are accessing this on the mobile app, please navigate back to the app to log in</p>";
                $('#consentModal').modal('show');        
               window.location.assign('./intro.php');
                //console.log("redirect");
            } else if (key === "") {
                //console.log("returning empty key");
            } else {
                giveFeedback(key, value, true);
                hidemodal = 1;
            }
        });
        if (parseInt(hidemodal) === 1) {
            $('#consentModal').modal('hide');
        }
    }, data);
}
...
function validateCopy(copy_pass, pass) {
    ...
    if (minlength.test(pass)){
        ...
        giveFeedback('password', "Password must be at least 8 characters long and contain at least 1 letter and 1 number", true);
    }
...
function getConsent(action, data){
    doXHR('./checkconsent.php',function (){
        var result = this.responseText;
        //console.log('Result: ', result);
        var consent = '';
        var valid = 1;
    ...
    //only show modal popup if the data on the first form is valid
        if (valid == 1 && (consent == 1||consent == 0)){
            ...
            getconsenttxt.push( "<div class='col-xs-3'><strong>* = mandatory</strong></div></div>");
            if (consent == 1) {;
                ...
                getconsenttxt.push( "<div class='col-sm-2 col-xs-12 text-center'><div class='checkbox'><input type='checkbox' value='1' id='privacy'>*<br></div></div></div>");
                ...
                getconsenttxt.push( "<div class='col-sm-2 col-xs-12 text-center'><div class='checkbox'><input type='checkbox' value='1' id='cookies'>*<br></div></div></div>");
                ...
                getconsenttxt.push( "<div class='col-sm-2 col-xs-12 text-center'> <div class='checkbox'> <input type='checkbox' value='1' id='e_consent_v'><br>*</div></div></div>");
               ...
               //GP records
                ...
                getconsenttxt.push( "<div class='col-sm-2 col-xs-12 text-center'><div class='checkbox' ><input type='checkbox' value='1' id='e_consent_gp'><br></div></div></div>");
                //agree to contact
                ...
                getconsenttxt.push( "<div class='col-sm-2 col-xs-12 text-center'><div class='checkbox'><input type='checkbox' value='1' id='e_consent_t'><br></div></div></div>");
                getconsenttxt.push( "<div class='row'>  <div class='col-sm-1'></div><div class='col-xs-12 col-sm-9'><p>I have read and understood the <a href='./privacy.php' target='_blank'>privacy policy</a></p></div>");
                getconsenttxt.push( "<div class='col-sm-2 col-xs-12 text-center'><div class='checkbox'><input type='checkbox' value='1' id='privacy'>*<br></div></div></div>");
                getconsenttxt.push( "<div class='row'>  <div class='col-sm-1'></div><div class='col-xs-12 col-sm-9'><p>I have read and understood the <a href='./cookies.php' target='_blank'>cookies policy</a></p></div>");
                getconsenttxt.push( "<div class='col-sm-2 col-xs-12 text-center'><div class='checkbox'><input type='checkbox' value='1' id='cookies'>*<br></div></div></div>");
                getconsenttxt.push( "<div class= 'row'><div class='col-sm-1'></div><div class='col-xs-11'><a href='#' data-toggle='tooltip' title=''><strong> Please complete this information</strong></a></div></div>");
                getconsenttxt.push( "<div class='row'> <div class='col-md-1'></div> <div class='col-md-3 col-xs-12'><strong> Gender </strong>");
                getconsenttxt.push( "<div class='radio'><label><input type='radio' value='F' name='gender'> Female</label></div>");
                getconsenttxt.push( "<div class='radio'><label><input type='radio' value='M' name='gender'> Male</label></div></div>");
                getconsenttxt.push( "<div class='col-md-4 col-xs-12'><strong> Ethnicity </strong>");
                getconsenttxt.push( "<div class='radio'><label><input type='radio' value='W' name='ethnicity'> White</label></div>");
                getconsenttxt.push( "<div class='radio'><label><input type='radio' value='M' name='ethnicity'> Mixed/multiple ethnicities</label></div>");
                getconsenttxt.push( "<div class='radio'><label><input type='radio' value='A' name='ethnicity'> Asian/Asian British</label></div>");
                getconsenttxt.push( "<div class='radio'><label><input type='radio' value='B' name='ethnicity'> Black/African/Caribbean/Black British</label></div>");
                getconsenttxt.push( "<div class='radio'><label><input type='radio' value='O' name='ethnicity'> Other ethnic group</label></div></div>");
                getconsenttxt.push( "<div class='col-md-4 col-xs-12'><strong> Age </strong>");
                getconsenttxt.push( "<div class='radio'><label><input type='radio' value='40' name='age'> 40-59 years</label></div>");
                getconsenttxt.push( "<div class='radio'><label><input type='radio' value='60' name='age'> 60-74 years </label></div>");
                getconsenttxt.push( "<div class='radio'><label><input type='radio' value='75' name='age'> 75 years and older</label></div></div></div>");
                getconsenttxt.push( "<span id='modal_feedback'></span>");
            };
            getconsenttxt.push('</div></form></div> <div class="modal-footer">');
            getconsenttxt.push('<button type="button" class="btn btn-default" onclick="continueConsent(\''+ action +'\',\''+ data +'\',\''+ consent+'\')"> Continue </button></div>');
            //console.log(data);
            consentdialog = getconsenttxt.join("");
...
function continueConsent(action, data, result){
    ...
    if (document.getElementById("e_consent_a").checked) {
        var e_consent_a = 1;
    } else {
        var e_consent_a = 0;
    }
    ...
    if (result == 1 && privacy == 1 && cookies == 1 && e_consent_a == 1) {
		...
		if (document.getElementById("e_consent").checked) {
            var e_consent = 1;
        } else { 
            var e_consent = 0;
        }
        if (document.getElementById("e_consent_gp").checked){
            var e_consent_gp = 1;
        } else { 
        	var e_consent_gp = 0;
        }
		if (document.getElementById("e_consent_v").checked){
			var e_consent_v = 1;
	    } else {
	        var e_consent_v = 0;
	    }
        if (document.getElementById("e_consent_t").checked) {
            var e_consent_t = 1;
        } else {
            var e_consent_t = 0;
        }
    ...
    if (consented == 1) {
        ...
        document.getElementById('modal_feedback').innerHTML="<strong>Please fill out all the fields</strong>";
    }
}
